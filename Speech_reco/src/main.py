...
def on_agent_event_callback(event, uuid, name, event_data, my_data):
    try:
        ...
        assert isinstance(agent_object, SpeechReco)
...
# inputs
def push_to_talk_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, SpeechReco)
        r = sr.Recognizer()
        ...
        try:
            ...
            igs.output_set_string("word", texte)
...
if __name__ == "__main__":
    ...
    igs.input_create("push_to_talk", igs.IMPULSION_T, None)

    igs.output_create("word", igs.STRING_T, None)

    igs.observe_input("push_to_talk", push_to_talk_input_callback, agent)
